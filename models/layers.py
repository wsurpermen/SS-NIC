...
def get_act(config):
  ...
  if config.model.activation.lower() == 'elu':
    return nn.ELU()
  elif config.model.activation.lower()  == 'selu':
      return nn.SELU()
...
class ConcatSquashConditionYLinear(nn.Module):
    def __init__(self, dim_in, dim_out, outputsize):
        super(ConcatSquashConditionYLinear, self).__init__()
        self.outputsize = outputsize
        self._layer = nn.Linear(dim_in, dim_out)
        self._hyper_bias = nn.Linear(1, dim_out, bias=False)
        self._hyper_gate = nn.Linear(1, dim_out)
        self._hyper_bias_y = nn.Linear(outputsize, dim_out, bias=False)
        self._hyper_gate_y = nn.Linear(outputsize, dim_out)


    def forward(self, t, x, y):
        return self._layer(x) * torch.sigmoid(self._hyper_gate(t.view(-1, 1)))\
               * torch.sigmoid(self._hyper_gate_y(y.view(-1, self.outputsize))) \
            + self._hyper_bias(t.view(-1, 1))\
            + self._hyper_gate_y(y.view(-1, self.outputsize))
