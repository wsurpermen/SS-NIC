...
class VESDE(SDE):
  def __init__(self, sigma_min=0.01, sigma_max=50, N=1000):
    ...
    self.discrete_sigmas = torch.exp(torch.linspace(np.log(self.sigma_min), np.log(self.sigma_max), N)).to(self.device)
    self.N = N
  ...
  def marginal_prob(self, x, t):  # return time step 0.01  and sigma_max
  ...
  def discretize(self, x, t):
    ...
    self.discrete_sigmas = torch.tensor(self.discrete_sigmas).to(t.device)
    timestep = (t * (self.N - 1) / self.T).long()
