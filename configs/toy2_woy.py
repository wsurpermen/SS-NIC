...
def get_config():
  ...
  config.training.label = False       # if train the feature only, only read the feature without label y
  config.training.xscore_label = True # if train the combine distribution p(x,y)
  config.training.cond_y = False     # if train the feature condition with y, set xscore_label==False
