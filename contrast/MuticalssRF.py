...
def add_flip_noise(dataset, noise_rate):
    ...
    for i in range(len(label_cat)):
        ...
        noise_index_list = []  # Record all noise subscripts
        n_index = 0
        while True:
            #  select the index  at each time
            rand_index = int(random.uniform(0, n))
            # If the subscript already exists, perform the next while
            if rand_index in noise_index_list:
                continue
            # Positive class and insufficient noise:flip
...
#  Load data, call add noise, only add noise to the training set,
...
def homo(train_data, k):
    ...
    for a in n:
        ...
        homo_list = pd.concat([homo_list, pd.DataFrame(data)])
    return homo_list
...
def hete(data, k):
    ...
    for a in n:
        ...
        dis = [[0] * len(data_1) for _ in range(len(data_2))]  # for the different density of DATa_2
        ...
        hete_list = pd.concat([hete_list, pd.DataFrame(data_2)])
    return hete_list
...
def return_predict(train_data, k=3, p=1):
    homo_data = homo(train_data, k)  # The same density
    df = hete(homo_data, k)  # the different density
    df['relative'] = df['homo'] / df['hete']  # rd: the different density/The same density
    return (df['relative'] >=p).values
