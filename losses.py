...
def get_sde_loss_fn(sde, train, reduce_mean=True, continuous=True, likelihood_weighting=True, eps=1e-5, cond_y=False,config=None):
  reduce_op = torch.mean if reduce_mean else lambda *args, **kwargs: 0.5 * torch.sum(*args, **kwargs)

  def loss_fn(model, batch):
    score_fn = mutils.get_score_fn(sde, model, train=train, continuous=continuous, cond_y=cond_y,config=config)
    t = torch.rand(batch.shape[0], device=batch.device) * (sde.T - eps) + eps  # return a uniform distribution
    z = torch.randn_like(batch)  # return a normal distribution
    mean, std = sde.marginal_prob(batch, t)
    if cond_y:  # in this case, the last column is label y
        z[:, -config.data.output_size:] = 0
    perturbed_data = mean + std[:, None] * z     # std's dimension == 1, std[:,None]'dimension == 2 and z's dimension==2
    if cond_y:  # in this case, the last column is label y, remove this column to calculate loss
      z = z[:, :-config.data.output_size]
...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True,
                likelihood_weighting=False, workdir=False, spl=True, writer=None,
                alpha0=None, beta0=None, cond_y=False,config=None):
  if continuous:
    loss_fn = get_sde_loss_fn(sde, train, reduce_mean=reduce_mean,
                              continuous=True, likelihood_weighting=likelihood_weighting, cond_y=cond_y, config=config)
  else:
